...
const Profile = () => {
  ...
  useEffect(() => {
    setPayForm((prev) => ({
      ...prev,
      data: { ...prev.data, phone: currentUser?.phone },
    }));
  }, [currentUser]);
  ...
  useEffect(() => {
    const getUserToUpdate = async () => {
      try {
        const res = await fetch(
          ...
          }/api/v1/auth/user-to-update/${searchParams.get("id")}`,
          {
            method: "GET",
          }
        );
    ...
    if (searchParams.get("id")) [getUserToUpdate()];
  }, [searchParams]);
  ...
  const sortedTransactions = [...(user?.wallet?.transactionHistory || [])].sort(
    (a: any, b: any) => {
  ...
  if (user) {
    return (
      <section className="flex flex-col gap-4 w-full min-h-screen bg-gray-950">
