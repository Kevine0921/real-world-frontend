...
function GroupSettings({}: Props) {
    ...
    try {
      if (file) {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/groups/upload-group-picture`,
          {
            method: "PUT",
            headers: {
  ...
  const changeVisibility = async (current_vis: string) => {
    try {
      ...
      const res = await updateGroup({
        group_state: current_vis,
        group_id: group?._id as string,
      });
    } catch (error) {
  ...
  const handleLeaveGroup = async () => {
    try {
      ...
      const accessToken = Cookies.get("access-token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/groups/leave/${group?._id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const handleDeleteGroup = async () => {
    try {
      ...
      const accessToken = Cookies.get("access-token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/groups/${group?._id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <div className="overflow-auto bg-gray-800">
      ...
      {group?.created_by[0]?._id === currentUser?._id && (
        <div className="p-3 border-b flex flex-col md:flex-row items-start justify-around">
          ...
          <div className="flex flex-col gap-2 items-end md:w-1/2 w-full">
            ...
            <div className="w-full flex flex-col gap-2">
              <label className="font-extrabold text-[.8rem]" htmlFor="tags">
                Group tags (separate by comma)
              </label>
              <textarea
                id="tags"
                name="tags"
                ...
                onChange={handleChange}
                className="w-full bg-transparent p-2 border outline-none"
                placeholder="Group Tags (separate by ,)..."
              />
            </div>

            <Select
              onValueChange={(e) =>
                setGroupData((prev) => ({ ...prev, group_type: e }))
              }
              defaultValue={groupData.group_type}
            >
      ...
      <div className="p-3 border-b  w-full">
        <h1 className="p-2 text-[1.2rem] font-extrabold">
          ...
          <div className="flex w-full justify-between items-center">
            ...
            <div>
              <Select>
                ...
                <SelectContent className="bg-white">
                  <SelectItem className="cursor-pointer" value="push">
                    push
                  </SelectItem>
            ...
            <div>
              <Select>
                ...
                <SelectContent className="bg-white">
                  ...
                  <SelectItem className="cursor-pointer" value="mentions">
                    Only @mentions
                  </SelectItem>
                  <SelectItem className="cursor-pointer" value="none">
                    Don't push message Notifications
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
        ...
        <div className="flex flex-col gap-3 w-full mt-5">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent className="bg-white">
                    ...
                    <SelectItem className="cursor-pointer" value="Invite-Only">
                      Secret
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            <div className="flex w-full justify-between items-center">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-[.8rem]">
                  {group?.isSacco
                    ? "This group is already a SACCO"
                    : `You need at least 5 members of a group to transition to sacco. Currently you have ${group?.members.length} members`}
                </p>
              </div>
              <div>
                <Button
          ...
          <div className="flex w-full justify-between items-center">
            ...
            <Button
              ...
              onClick={() => {
                navigator.clipboard
                  .writeText(group?.invite_link as string)
                  .then(() => {
      ...
      <div className="p-3 border-b  w-full ">
        ...
        <div className="flex flex-col gap-3 w-full mt-5 mb-11">
          <div>
            <h1>
              <span className="font-bold">Name:</span> {group?.group_name}
            </h1>
          </div>

          <div>
            <h1>
              <span className="font-bold">State:</span> {group?.group_state}
            </h1>
          </div>
          ...
          <div>
            <h1>
              ...
              {new Date(group?.createdAt as Date).toLocaleDateString()}
            </h1>
          </div>
        </div>
      </div>
