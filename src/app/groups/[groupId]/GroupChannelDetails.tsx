...
function ChannelDetails({}: Props) {
  ...
  const handleUpgrade = async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/groups/create-checkout-session`,
      {
        method: "POST",
        headers: {
    ...
    <>
      <Popover>
        <PopoverTrigger className="w-full">
          ...
          <PopoverContent className="text-white bg-gray-800 shadow-2xl p-0 z-40 gap-1 flex flex-col border-transparent border-l-8 border-l-neutral-400 pl-3 w-auto">
            {filteredMenuItems.map((item, index) => (
              <PopoverClose key={index}>
                <Link
                  ...
                  href={item.link}
                  key={index}
                  className="text-white flex p-2 w-full text-[0.9rem] hover:bg-gray-700 cursor-pointer rounded-md items-center gap-2 duration-100"
                >
                  {item.icon}
                  {item.name}
                </Link>
      ...
      <div className="w-full p-2 flex flex-col mt-5 h-full overflow-y-auto absolute">
        <span className="flex items-center justify-between text-[0.9rem]">
          ...
          <Dialog>
            ...
            <DialogContent className="bg-gray-900">
              <DialogHeader className="font-bold text-[1.2rem] text-white">
                Create a new channel
              </DialogHeader>
              <div className="w-full flex flex-col gap-2">
                <div className="w-full flex items-center justify-start">
                  <span className="p-2 grid place-content-center text-white">
                    {channelFields.state === "public" ? (
                      <Select
                        defaultValue="globe"
                        value={channelFields.channelIcon}
                        name="channelIcon"
                        onValueChange={(v) =>
                      ...
                      >
                        <SelectTrigger className="bg-gray-700 w-full text-white">
                          <SelectValue className="w-[10%]">
                            {channelFields.channelIcon === "globe" ? (
                              <Globe className="size-4" />
                            ) : channelFields.channelIcon === "lock" ? (
                              <Lock className="size-4" />
                            ) : channelFields.channelIcon === "hash" ? (
                        ...
                        <SelectContent className="bg-gray-700 text-white">
                          <SelectItem className="cursor-pointer" value="globe">
                            <Globe className="size-4" />
                          </SelectItem>
                          <SelectItem className="cursor-pointer" value="lock">
                            <Lock className="size-4" />
                          </SelectItem>
                          <SelectItem className="cursor-pointer" value="hash">
                            <Hash className="size-4" />
                          </SelectItem>
                        </SelectContent>
                      </Select>
        ...
        <div className="flex flex-col gap-4">
          <div className="w-full p-2 flex flex-col gap-2">
            <h1 className="text-[1rem] sm:text-[0.9rem] text-gray-400">
              Public channels
            </h1>
                ...
                (channel, index) =>
                  channel.state?.toLowerCase() === "public" && (
                    <div
                      ...
                      className={`flex items-center gap-2 w-full hover:bg-gray-600 hover:text-gray-300 duration-100 cursor-pointer p-1 rounded-md ${
                    ...
                    >
                      ...
                      {countUnreadMessages(channel) > 0 ? (
                        <span className="bg-neutral-500 w-7 flex items-center justify-center h-7 rounded-full text-white">
      ...
      <div className="absolute bottom-0 w-full bg-slate-00 h-auto flex items-center my-2 justify-start gap-2 p-2 border-orange-500 border rounded-3xl">
        <Avatar>
          <AvatarImage
