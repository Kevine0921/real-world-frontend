...
function DMProfile({}: Props) {
  ...
  const { setIsSideBarOpen, isSideBarOpen, windowWidth } =
    useContext(GroupContext);
  return (
    <div
      className={`w-[25%] ${
        isSideBarOpen && windowWidth! <= 1025
          ? "block w-full absolute top-0 left-0 h-full bg-blue-500"
          : "hidden "
      } lg:block bg-gray-900 overflow-auto p-2 z-50`}
    >
      {isSideBarOpen && (
        <span
      ...
      {currentDM ? (
        <div className="flex flex-col p-3 items-center text-center gap-4 ">
          <Avatar className="w-[100px] h-[100px]">
            <AvatarImage
              src={currentDM?.memberDetails[1]?.profile_pic}
          ...
          <div className="flex flex-col ">
            <h1 className="text-[1.2rem] font-bold">
              {currentDM?.memberDetails[1]?.lastName}{" "}
              {currentDM?.memberDetails[1]?.firstName}
            </h1>
          ...
          <div className="bg-gray-800 w-full p-3 rounded-md">
            <h1>
              Joined{" "}
              {moment(currentDM?.memberDetails[1]?.createdAt as Date).format(
                "MMM D, YYYY"
              )}
            </h1>
          </div>
        ...
        <div className="flex flex-col p-3 items-center text-center gap-4 ">
          ...
          <div className="bg-[#013a6fa6] w-full p-3 rounded-md">
            <h1>
              Joined{" "}
              {moment(currentUser?.createdAt as Date).format("MMM D, YYYY")}
            </h1>
          </div>
        </div>
      )}
