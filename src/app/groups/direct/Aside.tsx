...
function Aside({}) {
  const { group, isMemberListOpen, windowWidth, setIsMemberListOpen } =
    useContext(GroupContext);
  const [q, setQ] = useState("");
  const { allUsers, currentUser, setCurrentDM, currentDM } =
  ...
  const handleGetUserDms = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/chatrooms/user`,
        {
          method: "POST",
          headers: {
      ...
      if (!data.status)
        return toast.error(data.errors || data.messaga || "failed");
  ...
  return (
    <div
      className={` bg-gray-800 h-full text-neutral-200 capitalize ${
        isMemberListOpen && windowWidth! <= 1025
          ? "w-full absolute top-0 left-0 h-full bg-blue-500 z-50"
          : "relative hidden"
      } sm:block`}
    >
      <div className="my-2 flex items-center rounded-3xl bg-gray-900 sticky top-0 z-20 p-2 justify-between text-neutral-200 w-full">
        {isMemberListOpen && (
          <span
      ...
      <div className="w-full p-2">
        <div className="w-full flex items-center justify-between">
          ...
          <Dialog>
            ...
            <DialogContent className="w-[20%] p-2 mt-5 bg-gray-950 text-white">
              <DialogHeader>
                <Input className="bg-gray-800" placeholder="Start a DM to" />
              </DialogHeader>
              {allUsers?.length! <= 0
                ? "no users found"
                      ...
                      <Popover>
                        <PopoverTrigger className="flex items-center justify-normal gap-5 relative">
                          <div className="flex items-center justify-normal gap-5 p-3 w-full cursor-pointer hover:bg-[#6bb7ff73] duration-150 rounded-lg">
                            ...
                            <PopoverContent className="bg-gray-900 text-white w-full flex flex-col items-center p-5 gap-2 border-transparent shadow-lg rounded-lg">
                              ...
                              <div>
                                {checkIsActive(onlineUsers, user) ? (
                                  <div className="flex items-center gap-2">
                                    <span className="w-3 h-3 border rounded-full bg-green-500"></span>{" "}
                                    online
                                  </div>
                                ) : (
                                  <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                className="bg-gray-800 text-white hover:bg-gray-700  place-self-start w-full"
                              >
                                Send message
                              </Button>
                            </PopoverContent>
        ...
        <hr className="my-3 rounded border border-gray-500" />
            ...
            userDMs.map((dm, index) => {
              ...
              return (
                <div
                  ...
                  key={index}
                  className={`dm-item flex items-center rounded-md p-2 hover:bg-[rgba(62,175,255,0.31)] cursor-pointer ${
                    currentDM?._id === dm?._id && "bg-[rgba(62,175,255,0.31)]"
                  }`}
                >
                  <img
