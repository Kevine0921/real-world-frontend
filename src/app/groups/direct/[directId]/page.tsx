...
import {
  ...
  User,
  UserSettings,
} from "@/types"; // Assuming you have these types
import {
  ArrowLeft,
  AtSign,
  Bell,
...
function Page({}: Props) {
  ...
  useEffect(() => {
    setValidUserNames(
      currentDM?.memberDetails.map((member) => `@${member.lastName}`) || []
    );
  }, [currentDM]);
  ...
  const getMessages = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/messages/${currentDM?._id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const getChatRoomData = async () => {
    try {
      ...
      console.log(
        "patners",
        data?.chatroom?.memberDetails,
        "currentUser",
        currentUser
      );
      const partners = data?.chatroom?.memberDetails.filter(
  ...
  useEffect(() => {
    const currentDMUnreadMessages = unreadMessagesRef.current.filter(
      (msg) => msg?.chatroom?._id === currentDM?._id && !msg?.isRead!
    );
    ...
    if (currentDMUnreadMessages) {
      ...
      const timeout = setTimeout(() => {
        if (unreadMessagesRef.current) {
          unreadMessagesRef.current = unreadMessagesRef.current.map(
            (unreadmsg: UnreadMessage) => {
              if (unreadmsg?.chatroom?._id === currentDM?._id)
                return {} as UnreadMessage;
              return unreadmsg;
            }
          );
        }
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      socket.emit("new-message", {
        sender: currentUser,
        chatroomId: currentDM?._id,
        sentTo: currentDM?._id,
        receiver: currentDM?.members,
        message: {
          ...data.message,
          sender: currentUser,
  ...
  return (
    <DragDrop setAttachments={setAttachments}>
      <div className="h-screen flex flex-col bg-gray-950 text-white">
        {fileUploading.state && (
          <div className="w-auto p-2 pl-5 pr-5 bg-gray-800 rounded-full shadow-lg text-white absolute left-1/2 mt-5">
            file (s) uploading ...
          </div>
        )}
        <div className="flex justify-between p-4 bg-gray-800 border-b border-gray-700 w-full">
          <div className="flex items-center gap-2 capitalize text-[1.2rem] text-xl">
            ...
            {currentPatners?.length === 1 ? (
              <div className="flex items-center gap-2">
                ...
                />
                <span>
                  {currentPatners[0].firstName} {currentPatners[0].lastName}
                </span>
              </div>
            ) : (
          ...
          <div className="flex items-center gap-2">
            <Popover>
              ...
              <PopoverContent className="text-white bg-gray-800 shadow-2xl z-50 flex flex-col pl-3 w-auto gap-3 ">
                ...
                <div className="w-full flex items-center justify-between gap-5">
                  <label htmlFor="block">
                    {userSettings?.notificationSettings.chatroomsBlocked.includes(
                      currentDM?._id!
                    )
                      ? "Unblock"
                      : "block"}
                      ...
                      handleUpdateUserSettings({
                        ...
                        notificationSettings: {
                          ...
                          chatroomsBlocked: e.target.checked
                            ...
                            : userSettings.notificationSettings.chatroomsBlocked.filter(
                                (chat) => chat !== currentDM?._id
                              ),
                        },
                      })
                    }
                    checked={userSettings?.notificationSettings.chatroomsBlocked.includes(
                      currentDM?._id!
                    )}
                    className="p-5 w-5 h-5 cursor-pointer rounded-full "
                  />
            ...
            <Dialog>
              ...
              <DialogContent className="bg-gray-900 p-2 flex flex-col w-[30%] h-[70%]">
                ...
                <div className="mt-5 h-[500px] overflow-auto">
                  {!queriedMessages.length
                    ...
                    : queriedMessages.map((message) => {
                        return (
                          <div
                            className="w-full flex cursor-pointer hover:bg-gray-600 border-b rounded-md  p-2 flex-col"
                            key={message._id}
                          >
                ...
                return (
                  <React.Fragment key={msg._id}>
                    ...
                    <div
                      ...
                      className={`flex gap-4 hover:bg-[#cbcbcb2e] cursor-pointer rounded-md items-start mb-1 justify-normal ${
                        index === msgs.length && "mb-5"
                      } ${
                        msg.pinned ? "bg-[rgba(255,193,59,0.42)]" : " "
                      } group`} // Reduced margin between consecutive messages
                    >
                      {msg.pinned && <Pin />}
                      {showAvatarAndName ? (
                            ...
                            <div className="W-full flex flex-col border-gray-700 border focus-within:border-white rounded-md">
                              ...
                              <div className="">
                                <textarea
                                  ...
                                  value={isEditing.content.replace(
                                    /<\/?[^>]+(>|$)/g,
                                    ""
                                  )}
                                  onChange={(e) =>
                              ...
                              <div className="w-full flex p-2">
                                <div className="">
                                  <Popover>
                                    ...
                                    <PopoverContent className="text-white bg-gray-800 shadow-2xl z-50 gap-1 flex flex-col ">
                                      <input
                                        type="file"
                                        hidden
                                        name="attachment"
        ...
        {userSettings?.notificationSettings.chatroomsBlocked.includes(
          ...
          <div className="W-full p-2 text-center">You blocked this profile</div>
        ) : (
              ...
              <div className="W-full flex flex-col border-gray-700 border focus-within:border-white rounded-md">
                ...
                <div className="relative">
                  <ChatInput
                    ...
                    placeholder={
                      currentPatners?.length > 0
                        ? `message ${currentPatners[0]?.lastName} ...`
                        : "Send DM..."
                    }
