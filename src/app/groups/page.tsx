...
function Groups({}: Props) {
  ...
  const filterGroups = () => {
    ...
    if (selectedCategory) {
      filtered = filtered.filter(
        (group) => group.group_type === selectedCategory
      );
    }
  ...
  const joinPublicGroup = async (group: GroupTypes) => {
    try {
      if (!currentUser?.is_complete && group?.isSacco)
        return toast.error("Complete your profile first to join.");
      ...
      if (res._id !== null) {
        socket.emit("new-group-join", {
          receiver: group.members,
          joined_user: currentUser,
        });
  ...
  const sendRequestToJoin = async (group: GroupTypes) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/groups/requests`,
        {
          method: "POST",
          headers: {
      ...
      if (data.status) {
        toast.success(
          "Request sent successfully. Wait for approval from admins"
        );
        setFormattedGroups((prev) =>
          prev?.filter((prevGroup) => prevGroup?._id !== group._id)
        );
      } else {
  ...
  return (
    <div className="flex flex-col w-full h-screen overflow-hidden bg-gray-950">
      {/* Sticky Header */}
                  ...
                  {categoryList.map((category, index) => (
                    <li key={index} className="w-full block ">
                      <button
                        ...
                        } p-2 w-full rounded-md hover:bg-gray-200 hover:text-black text-white duration-200`}
            ...
            {categoryList.map((category, index) => (
              <li key={index} className="w-full sm:w-auto">
                <button
                  className={`${
                    selectedCategory === category.link ? "" : ""
                  } p-2 text-sm sm:text-base  rounded-md hover:bg-gray-200 hover:text-black duration-200`}
                  onClick={() => setSelectedCategory(category.link)}
                >
                  {category.name}
                </button>
      ...
      <div className="h-full overflow-auto flex flex-col">
        <div className="mt-5">
          <h1 className="p-5 text-[2rem] text-white">Browse Groups</h1>
          ...
          {isLoading && (
            <Skeleton
              ...
              height={300}
              count={5}
              className="h-[300px] sm:w-[45%] md:w-[30%] lg:w-[23%]"
            />
          )}
      ...
      p-4 mb-4 shadow-lg hover:shadow-xl transition-all duration-300 
      bg-[#2078ca39] text-white rounded-lg 
      flex flex-col gap-3 
              ...
              >
                ...
                <div className="relative h-[150px] w-full overflow-hidden rounded-md">
                  <img
                    src={
                      group.group_picture ||
                      "https://res.cloudinary.com/djehh7gum/image/upload/v1729070790/ura0gnomuhpti7sbi79r.png"
                    }
                ...
                <div className="flex-grow flex flex-col gap-2">
                  ...
                  <p className="text-sm text-gray-200 flex-grow line-clamp-2">
                  ...
                  <div className="text-sm text-gray-300">
                    {group.memberCount}{" "}
                    {group.memberCount !== 1 ? "members" : "member"}
                  </div>
                </div>
                ...
                <Button
                  variant="secondary"
                  className="w-full mt-auto bg-gray-700 hover:bg-gray-600 text-white transition-colors"
                  onClick={() =>
                    group.group_state === "Public"
                      ? joinPublicGroup(group)
                      : sendRequestToJoin(group)
                  }
                >
                  {group.group_state === "Public"
                    ? "Join Group"
