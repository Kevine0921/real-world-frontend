...
function Editor({
  ...
  return (
    <div className="flex bg-gray-800 flex-col w-full relative">
      {isMentioning && (
        <ul className="mention-dropdown absolute bg-gray-700 text-white w-1/5 rounded-md overflow-auto z-50 max-h-44 shadow-md">
          {filteredUsers.map((user: User) => (
            <>
              <li
                className="cursor-pointer hover:bg-gray-600 p-3 hover:text-black"
                key={user.id}
                onClick={() => {
      ...
      <div className="flex w-full flex-col rounded-md overflow-hidden bg-gray-800 pt-2 px-2">
        <div
          ref={containerRef}
          className="max-h-auto min-h-[50px] bg-gray-900 w-[100%]  placeholder:text-white rounded-full"
        />
      </div>
      <div className="flex text-black bg-gray-800 justify-between overflow-hidden">
          ...
          <div className="px-2">
            {variant === "create" && (
              <>
                ...
                <button
                  disabled={disabled}
                  className="p-1 font-bold rounded-full cursor-pointer"
                >
                  <label
            ...
            <EmojiPopover onEmojiSelector={onEmojiSelector}>
        ...
        <div className="justify-self-end">
          {variant === "update" && (
            <>
              <Button className="text-white" disabled={disabled}>
                Cancel
              </Button>
              <Button className="text-white" disabled={disabled || isEmpty}>
                Save
              </Button>
            </>
          )}
      ...
      {/* {variant === "create" && (
        <div
          className={cn(
            " bg-gray-800 text-[10px] text-muted-foreground justify-end w-full flex opacity-0",
            !isEmpty && "opacity-100"
          )}
        >
          <p className="text-[.7rem]">
            <code className="text-[.6rem]">shift+enter</code> to add a new line
          </p>
        </div>
      )} */}
    </div>
